...
public class LogicFloatSlotHandler : ILogicSlotHandler
{
    ...
    public string GetDisplayText(LogicSlotValue value)
    {
        ...
        {
            LogicValueSourceType.Constant => value.floatValue.ToString("0.##"),
            LogicValueSourceType.RandomRange => $"随机 {value.floatValue:0.##} ~ {value.floatValueMax:0.##}",
    ...
    public bool IsValid(LogicSentenceTemplate.SlotDefinition slot, LogicSlotValue value)
    {
        return value.sourceType switch
        {
            ...
            LogicValueSourceType.RandomRange => value.floatValue <= value.floatValueMax,
    ...
    public void DrawEditor(LogicSentenceTemplate.SlotDefinition slot, LogicSlotValue value)
    {
        switch (value.sourceType)
        {
            case LogicValueSourceType.Constant:
                value.floatValue = EditorGUILayout.FloatField("浮点数", value.floatValue);
                break;

            case LogicValueSourceType.RandomRange:
                value.floatValue    = EditorGUILayout.FloatField("最小值", value.floatValue);
                value.floatValueMax = EditorGUILayout.FloatField("最大值", value.floatValueMax);
                break;
            ...
            case LogicValueSourceType.Expression:
                LogicExpressionSlotEditorGUI.DrawExpressionEditor(LogicSlotValueType.Float, value);
                break;
        }
    }
